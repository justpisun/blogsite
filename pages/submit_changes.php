<?php
    ...
    if(mb_strlen($description) != 0 && (mb_strlen($description) >= 30)){
        echo "<h1>Описание слишком маленькое или слишком большое</h1>";
        exit();
    }
    
    if(mb_strlen($email) != 0 && (mb_strlen($email) <= 7 || mb_strlen($email) >= 50)){
        echo "<h1>Email не существует</h1>";
        exit();
    }

    if(mb_strlen($first_name) != 0 && (mb_strlen($first_name) <= 2 || mb_strlen($first_name) >= 30)){
        echo "<h1>Имя не существует</h1>";
        exit();
    }

    if(mb_strlen($second_name) != 0 && (mb_strlen($second_name) <= 2 || mb_strlen($second_name) >= 30)){
    ...
    if(mb_strlen($country) != 0 && (mb_strlen($country) <= 4 || mb_strlen($country) >= 20)){
        echo "<h1>Введите страну правильно</h1>";
        exit();
    }

    if(mb_strlen($city) != 0 && (mb_strlen($city) <= 2 || mb_strlen($city) >= 40)){
